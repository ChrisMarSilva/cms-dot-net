...
public class CartRepository : ICartRepository
{
    ...
    public async Task<bool> CleanCartAsync(string userId)
    {
        ...
        _context.CartItems.RemoveRange(_context
            .CartItems
    ...
    private async Task UpdateQuantityAndItems(
        CartDto cartDto, 
        Cart cart, 
        CartHeader? cartHeader)
    {
        //Se CartHeader não é null - verifica se CartItems possui o mesmo produto
        var cartItem = await _context
            .CartItems
            .AsNoTracking()
            .FirstOrDefaultAsync(p => p.ProductId == cartDto.CartItems.FirstOrDefault()!.ProductId  && p.CartHeaderId == cartHeader!.Id);
