...
public class CartController : Controller
{
    ...
    private async Task<CartViewModel?> GetCartByUser()
    {
        ...
        var cart = await _cartService
            .GetCartByUserIdAsync(userId: userId, token: token);
        ...
        if (cart?.CartHeader is not null)
        {
            if (!string.IsNullOrEmpty(cart.CartHeader.CouponCode))
            {
                var coupon = await _couponService
                    .GetDiscountCouponAsync(couponCode: cart.CartHeader.CouponCode, token: token);
    ...
    public async Task<IActionResult> RemoveItem(int id)
    {
        ...
        var result = await _cartService
            .RemoveItemFromCartAsync(cartId: id, token: token);
    ...
    public async Task<IActionResult> DeleteCoupon()
    {
        ...
        var result = await _cartService
            .RemoveCouponAsync(userId: userId, token: token);
